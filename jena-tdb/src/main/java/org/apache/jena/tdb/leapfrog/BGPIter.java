...
import org.apache.jena.tdb.leapfrog.LFTrieIndex;
...
public class BGPIter {
    ...
    private Triple[] triples;                         // Array of all triples from the BGP.
    ...
    private LFTrieIndex[][] itersForVar;              // Each itersForVar[i] contains all iterators containing the variable localAttributeOrder[i],
                                                      ...
                                                      // and the order is always maintained (the position of the minimum changes
                                                      // p=(p+1)%n after each seek).
    ...
    private int level = -1;                           // Current variable index in the local attribute order, if level = i, we know
                                                      // that variables 0...(i-1) already have a binding.
    private int baseLevel = 0;                        // how many variables in local will be set from outside.
    ...
    public boolean openTerms() {
        ...
        for (int i = 0; i < triples.length; i++) {
            Node nodes[] = { triples[i].getSubject(), triples[i].getPredicate(), triples[i].getObject() };
    ...
    /** calls up() in all the corresponding iterators for the current level */
    private void up() {
    ...
    private boolean findIntersection() {
        ...
        // Call next unless this is the first call to findIntersection() after open() was called
        if (beforeFirst) {
            beforeFirst = false;
        } else {
            currentIter[p].next();
        ...
        // normal modulus of negative number gives a negative result, use Math.floorMod to avoid this
    ...
    public Var[] init(Var[] upperVars) {
        ...
        for (int i = 0; i < triples.length; i++) {
            ...
            iter[i] = new LFTrieIndex(btree, triples[i], localAttributeOrder, permutation,
                    getNoIntersectionLevel(triples[i], permutation, upperVars));
        }
        ...
        for (int i = 0; i < localAttributeOrder.length; i++) {
            for (int lvl = 0; lvl < localAttributeOrder.length; lvl++) {
                // add to triplesForLevel all the iterators for the current lvl
     ...
     * @return the corresponding position in the indexes array, need to be
     *         consistent with the order declared at Names.java
     */
    ...
    public boolean tryOpenAndSeek(long nodeId) {
        ...
        for (int i = 0; i < it.length; i++) {
            it[i].open();
            if (it[i].key() != nodeId) { // could find the key at open, don't seek in that case
                it[i].seek(nodeId);
            }
            if (it[i].key() != nodeId) {
                res = false;
    ...
    public boolean seekBinding(BindingNodeId base, int firstChangedVarPos) {
        ...
        while (level + 1 < localAttributeOrder.length) {
            Var v = localAttributeOrder[level + 1];
            if (base.containsKey(v)) {
                if (!tryOpenAndSeek(base.get(v).getId())) {
                    up();
                    return false;
                }
            } else {
    ...
    public boolean hasVariable(Var v) {
        for (Triple triple : triples) {
            if (triple.getSubject().equals(v)
                ||triple.getPredicate().equals(v)
                || triple.getObject().equals(v)) {
                return true;
            }
        }
        return false;
