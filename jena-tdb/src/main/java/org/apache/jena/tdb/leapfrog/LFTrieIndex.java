...
public class LFTrieIndex {
    ...
    public LFTrieIndex(BPlusTree btree, Triple triple, Var[] localAttrOrder, int permutation, int noIntersectionLvl) {
        this.noIntersectionLevel = noIntersectionLvl;
        buffer = new long[BUFFER_SIZE][];

        pageMgr = btree.getRecordsMgr().getRecordBufferPageMgr();

        noIntersectionVars = new Var[3 - noIntersectionLvl];
        ...
        for (int i = noIntersectionLvl; i < 3; i++) {
            for (int j = 0; j < localAttrOrder.length; j++) {
                if (localAttrOrder[j].equals(noIntersectionVars[i - noIntersectionLvl])) {
    ...
    public void open() {
        ...
        if (level == 0) {
            ...
            currentRecordsNode = n.search(nodes, Record.zeroTripleRecord);
            ...
            Record recordMin = RecordFactory.tripleRecordFactory.create(min);
            ...
            while ((Record.keyLT(recordMin, n.getLowOrZeroRecord()) || Record.keyLT(n.getHighOrZeroRecord(), recordMin))
                    && !nodes.empty()) {
                n.release();
                n = nodes.pop();
            }
    ...
    public void next() {
        ...
        Record recordMin = RecordFactory.tripleRecordFactory.create(min);
        Record recordMax = (level == 0) ? null : RecordFactory.tripleRecordFactory.create(max);
    ...
    public void internalSearch(Record min, Record max) {
        ...
        if (r == null) {
            BPTreeNode n = nodes.pop();
            while (!nodes.empty()
                    && (Record.keyLT(n.getHighOrZeroRecord(), min) || Record.keyLT(min, n.getLowOrZeroRecord()))) {
                n.release();
                n = nodes.pop();
            }
    ...
    public void seek(long key) {
        // If the current position is already greater or equal it moves to the next
        // position anyways.
    ...
    public void startNoIntersection(long[] binding) {
        ...
        switch (level) {
        case -1:
            noIntersectionMin = RecordFactory.tripleRecordFactory.create(min);
        ...
        noIntersectionMin = RecordFactory.tripleRecordFactory.create(min);
        noIntersectionMax = (level == -1) ? null : RecordFactory.tripleRecordFactory.create(max);
        ...
        recordIter = (RecordRangeIterator) RecordRangeIterator.iterator(id, noIntersectionMin, noIntersectionMax,
                pageMgr);
        fillBuffer();
